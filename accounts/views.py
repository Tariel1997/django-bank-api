...
@extend_schema(
    tags=["Authentication"], description="Register a new user and create a bank account"
)
...
class WithdrawView(APIView):
    ...
    def post(self, request):
        ...
        if account.balance < amount:
            return Response(
                {"error": "Insuficient funds"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"message": "Withdrawal successful", "new_balance": str(account.balance)}
        )
